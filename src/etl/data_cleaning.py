...
data["Description"] = (
    data["Description"].str.lower().str.strip().replace(r"\s+", " ", regex=True)
)

# Generate a mapping of StockCode to Description
stockcode_to_description = (
    data.dropna(subset=["Description"]).groupby("StockCode")["Description"].first()
)

# For rows with missing descriptions, look up the StockCode in the mapping and fill the description accordingly
data["Description"] = data.apply(
    lambda row: stockcode_to_description.get(row["StockCode"], "Unknown")
